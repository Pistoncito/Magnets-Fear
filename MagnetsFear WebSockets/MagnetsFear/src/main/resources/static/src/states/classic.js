...
MagnetsFear.classicState.prototype = {
    ...
    create: function() {
        ...
        esfera1 = new Sphere();
        esfera2 = new Sphere();
        initSphere1(640 * player.id + 320, 360, 'sphere1');
        initSphere2(640 * opponent.id + 320, 360, 'sphere2');
    ...
    initProyectiles: function(num){
    	for(i=0; i< num; i++)
	    {	
    			proyectiles[i]= new Proyectile(proyectiles.create(640*i + 320, 
    					200,'proyectileSpSheet'));
    ...
    spawnBases: function (){
      ...
      for(i=0; i< n_bases; i++)
            {
              var posX1 = Math.round(pointX + R * Math.cos(dist * (angle + i)));
              var posY1 = Math.round(pointY + R * Math.sin(dist * (angle + i)));
              ...
              bases2[i]= new Bases(bases2.create((1280-posX1), (720-posY1),'civilization2'));
